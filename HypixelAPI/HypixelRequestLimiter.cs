...
public interface IHypixelRequestLimiter {
	...
	ValueTask<RateLimitLease> AcquireOneAsync() {
		return AcquireAsync(1);
	}

	ValueTask<RateLimitLease> AcquireOneAsync(CancellationToken cancellationToken) {
...
public class HypixelRequestLimiter(ILogger<IHypixelRequestLimiter> logger) : IHypixelRequestLimiter {
		...
		if (_limiter == null || _configuredPermitLimit != serverLimit)
			...
			lock (_lock) {
				...
				if (_limiter == null || _configuredPermitLimit != serverLimit) {
					...
					logger.LogWarning(
						"Hypixel API rate limit changed/initialized! New limit: {ServerLimit}, Remaining: {ServerRemaining}",
						serverLimit, serverRemaining);

					// Immediately sync the new limiter's remaining count.
					var permitsToBurn = serverLimit - serverRemaining;
					if (permitsToBurn > 0) newLimiter.AttemptAcquire(permitsToBurn);
	...
	public async ValueTask<RateLimitLease> AcquireAsync(int permitCount = 1,
		CancellationToken cancellationToken = default) {
		// Allow request through bootstrap limiter if the main limiter is not initialized yet.
		if (_limiter == null) return await _bootstrapLimiter.AcquireAsync(permitCount, cancellationToken);
