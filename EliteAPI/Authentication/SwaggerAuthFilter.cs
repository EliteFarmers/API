...
public class SwaggerAuthFilter : IOperationFilter {
	public void Apply(OpenApiOperation operation, OperationFilterContext context) {
		...
		if (!metadata.Any(x => x is AuthorizeAttribute || x is ServiceFilterAttribute filter && (filter.ServiceType == typeof(DiscordAuthFilter) || filter.ServiceType == typeof(DiscordBotOnlyFilter)))) 
		{
			return;
		}
