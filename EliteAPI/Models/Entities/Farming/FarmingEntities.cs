...
public class Farming
{
	...
	[Column(TypeName = "jsonb")] public FarmingInventory? Inventory { get; set; } = new();

	[Column(TypeName = "jsonb")] public FarmingFortune? Fortune { get; set; } = new();

	public Pests Pests { get; set; } = new();

	public double TotalWeight { get; set; } = 0;

	[Column(TypeName = "jsonb")] public Dictionary<string, double> CropWeight { get; set; } = new();

	[Column(TypeName = "jsonb")] public Dictionary<string, double> BonusWeight { get; set; } = new();

	[Column(TypeName = "jsonb")] public Dictionary<Crop, long> UncountedCrops { get; set; } = new();
