...
public static class FarmingWeightParser {
	...
	public static Dictionary<string, double> ParseCropWeight(JsonDocument jsonCollections,
		Dictionary<Crop, long> uncounted) {
	...
	public static Dictionary<Crop, double> ParseCropWeight(this Dictionary<Crop, long> collections,
		Dictionary<Crop, double>? weights = null, bool forEvent = false) {
	...
	private static Dictionary<string, double> ParseBonusWeight(ProfileMember member,
		IReadOnlyDictionary<string, int> craftedMinions) {
		...
		else if (farmingXp > 55172425) bonus.Add("Farming Level 50", config.Farming50Bonus);
		...
		if (obtained > 0) bonus.Add("Farming Minions", obtained * config.MinionRewardWeight);
	...
	public static double CountCropWeight(this CropCollection crops) {
		...
		try {
			...
			result += Math.Max(crops.Carrot - PestParser.CalcUncountedCrops(Pest.Cricket, crops.Cricket), 0) /
			          weights[Crop.Carrot];
			result += Math.Max(crops.CocoaBeans - PestParser.CalcUncountedCrops(Pest.Moth, crops.Moth), 0) /
			          weights[Crop.CocoaBeans];
			result += Math.Max(crops.Melon - PestParser.CalcUncountedCrops(Pest.Earthworm, crops.Earthworm), 0) /
			          weights[Crop.Melon];
			result += Math.Max(crops.NetherWart - PestParser.CalcUncountedCrops(Pest.Beetle, crops.Beetle), 0) /
			          weights[Crop.NetherWart];
			result += Math.Max(crops.Potato - PestParser.CalcUncountedCrops(Pest.Locust, crops.Locust), 0) /
			          weights[Crop.Potato];
			result += Math.Max(crops.Pumpkin - PestParser.CalcUncountedCrops(Pest.Rat, crops.Rat), 0) /
			...
			var mushroom = Math.Max(crops.Mushroom - PestParser.CalcUncountedCrops(Pest.Slug, crops.Slug), 0) /
			               weights[Crop.Mushroom];
			var doubleBreak = Math.Max(crops.Cactus - PestParser.CalcUncountedCrops(Pest.Mite, crops.Mite), 0) /
			                  weights[Crop.Cactus];
			doubleBreak += Math.Max(crops.SugarCane - PestParser.CalcUncountedCrops(Pest.Mosquito, crops.Mosquito), 0) /
			               weights[Crop.SugarCane];
