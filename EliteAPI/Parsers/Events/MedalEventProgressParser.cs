...
public static class MedalEventProgressParser {
	public static void UpdateMedalProgress(this MedalEventMember eventMember, MedalEvent @event, ProfileMember member) {
		...
		var participations = from participation in member.JacobData.Contests
			...
			group participation by participation.JacobContest.Timestamp
			into g
			select g.OrderByDescending(p => p.MedalEarned)
				.ThenByDescending(p => p.Collected)
				.First();
		...
		foreach (var participation in participations) {
			...
			if (!eventMember.Data.EarnedMedals.TryAdd(medal, 1)) eventMember.Data.EarnedMedals[medal]++;
		}
