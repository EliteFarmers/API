...
public static class EventProgressParser
{
	...
	public static void LoadProgress(this EventMember eventMember, DataContext context, ProfileMember member,
		Event @event) {
		var currentTime = DateTimeOffset.UtcNow;
		...
		switch (@event.Type) {
			...
			case EventType.Collection:
				if (eventMember is CollectionEventMember collectionMember &&
				    @event is CollectionEvent collectionEvent) {
		...
		void ApiAccessDisqualifyMember() {
			...
			if (eventMember.Team is not null) {
				...
				if (eventMember.Team?.Members.Count == 0) context.Entry(eventMember.Team).State = EntityState.Deleted;
			}
		}
	}
