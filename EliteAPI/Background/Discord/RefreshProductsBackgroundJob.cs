...
public class RefreshProductsBackgroundJob(
	...
	public async Task Execute(IJobExecutionContext executionContext) {
		...
		if (executionContext.RefireCount > 3) {
			messageService.SendErrorMessage("Failed to fetch Discord products",
				"Failed to fetch products from Discord");
			return;
		}
	...
	private async Task RefreshDiscordProducts(CancellationToken ct) {
		...
		if (products.Count == 0) await db.StringGetSetExpiryAsync("bot:products", TimeSpan.FromSeconds(60));
	...
	private async Task<List<DiscordProduct>> FetchDiscordProducts(CancellationToken ct) {
		...
		try {
			return await response.Content.ReadFromJsonAsync<List<DiscordProduct>>(ct) ?? [];
		}
		catch (Exception e) {
