...
public class GuildService(
	...
	public async Task<ActionResult>
		...
		if (guild is null) return new NotFoundObjectResult("Guild not found.");

		var channel = guild.Channels.FirstOrDefault(x => x.Id.ToString() == channelId);

		if (channel is null) return new NotFoundObjectResult("Channel not found.");
	...
	public async Task UpdateGuildData(ulong guildId, IncomingGuildDto guild) {
		...
		if (dbGuild is null) {
			var icon = guild.Icon is not null
				? await guildImageService.UpdateGuildIconAsync(guildId, guild.Icon)
				: null;
			var banner = guild.Banner is not null
				? await guildImageService.UpdateGuildBannerAsync(guildId, guild.Banner)
				: null;
	...
	public async Task UpdateGuildChannelData(ulong guildId, IncomingGuildChannelDto channel, bool skipSave = false) {
		...
		if (!skipSave) await context.SaveChangesAsync();
	}
