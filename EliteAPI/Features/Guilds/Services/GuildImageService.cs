...
{
    public async Task<Image?> UpdateGuildIconAsync(ulong guildId, string iconHash, Image? image = null, bool force = false) {
        try {
            if (image?.Hash == iconHash && !force) {
                return image; // Same hash means the image is already up to date
            }
    ...
    public async Task<Image?> UpdateGuildBannerAsync(ulong guildId, string bannerHash, Image? image = null, bool force = false) {
        try {
            if (image?.Hash == bannerHash && !force) {
                return image; // Same hash means the image is already up to date
            }
