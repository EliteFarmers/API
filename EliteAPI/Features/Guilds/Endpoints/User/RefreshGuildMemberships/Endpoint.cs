...
internal sealed class RefreshGuildMembershipsEndpoint(
	...
	public override void Configure() {
		...
		Summary(s => { s.Summary = "Refresh guild memberships for the current user"; });
	}
	...
	public override async Task HandleAsync(CancellationToken c) {
		...
		if (!user.GuildsLastUpdated.OlderThanSeconds(60))
			ThrowError("Guild memberships can only be refreshed once every 60 seconds",
				StatusCodes.Status429TooManyRequests);

		var result = await discordService.FetchUserGuilds(user);

		if (result.Count == 0) ThrowError("No guild memberships found", StatusCodes.Status404NotFound);
