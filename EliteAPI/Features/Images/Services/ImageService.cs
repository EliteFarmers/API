...
) : IImageService 
{
    public async Task<Image> ProcessAndUploadImageAsync(
        ...
        if (image.Frames.Count > 1) {
            ReduceFrameRate(image, maxFps: 8);
        }
    ...
    private async Task UploadImageVariantAsync(SixLabors.ImageSharp.Image image, string basePath, ImageVariantDefinition variant, Image imageEntity, CancellationToken token) {
    ...
    public async Task UpdateImageFromRemoteAsync(Image existingImage, string remoteUrl, string basePath, string presetName) {
        var formFile = await objectStorageService.DownloadRemoteImageAsync(remoteUrl);
