...
) : Endpoint<ReorderCategoryProductsRequest>
{
	public override void Configure() {
		...
		Summary(s => { s.Summary = "Reorder Products in Shop Category"; });
	}
	...
	public override async Task HandleAsync(ReorderCategoryProductsRequest request, CancellationToken c) {
		...
		if (products.Count != ordering.Count) AddError("Invalid number of products.");

		if (ordering.Select(e => e.Id).Except(products.Select(e => e.ProductId)).Any()) AddError("Invalid product id.");
		...
		try {
			for (var i = 0; i < ordered.Count; i++) {
				...
				if (product is null) ThrowError("Invalid product id.");

				if (product.Order == i) continue;

				await context.ProductCategories
					.Where(e => e.CategoryId == request.CategoryId && e.ProductId == product.ProductId)
					.ExecuteUpdateAsync(e => e.SetProperty(x => x.Order, i), c);
			}
		}
		catch {
