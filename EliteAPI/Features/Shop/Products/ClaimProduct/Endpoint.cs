...
internal sealed class ClaimProductEndpoint(
	...
	public override void Configure() {
		...
		Summary(s => { s.Summary = "Claim Free Shop Product"; });
	}

	public override async Task HandleAsync(DiscordIdRequest request, CancellationToken c) {
		var userId = User.GetDiscordId();
		if (userId is null) ThrowError("Unauthorized", StatusCodes.Status401Unauthorized);
		...
		if (!product.Available) ThrowError("Not available", StatusCodes.Status400BadRequest);

		if (product.Price != 0) ThrowError("Product is not free!", StatusCodes.Status400BadRequest);
