...
internal sealed class GetBadgesEndpoint(
	...
	public override void Configure() {
		...
		Summary(s => { s.Summary = "Get all badges"; });

		Description(x => x.AutoTagOverride("Badge"));

		Options(o => { o.CacheOutput(c => c.Expire(TimeSpan.FromHours(2)).Tag("badges")); });
	}
	...
	public override async Task HandleAsync(CancellationToken c) {
		...
		await Send.OkAsync(mapper.Map<List<BadgeDto>>(badges), c);
	}
}