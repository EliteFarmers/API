	...
	: IMonetizationService
{
	...
	public async Task UpdateProductAsync(ulong productId, EditProductDto editProductDto) {
		...
		if (!product.Available && editProductDto.Available is true) product.ReleasedAt = DateTimeOffset.UtcNow;

		product.Available = editProductDto.Available ?? product.Available;

		if (!string.IsNullOrWhiteSpace(editProductDto.ReleasedAt) &&
		    long.TryParse(editProductDto.ReleasedAt, out var unixTime))
			try {
		...
		if (editProductDto.Features is not null) {
			product.Features.MaxJacobLeaderboards =
				editProductDto.Features.MaxJacobLeaderboards ?? product.Features.MaxJacobLeaderboards;
			product.Features.MaxMonthlyEvents =
				editProductDto.Features.MaxMonthlyEvents ?? product.Features.MaxMonthlyEvents;
			...
			product.Features.HideShopPromotions =
				editProductDto.Features.HideShopPromotions ?? product.Features.HideShopPromotions;
			product.Features.WeightStyleOverride =
				editProductDto.Features.WeightStyleOverride ?? product.Features.WeightStyleOverride;
	...
	public async Task FetchUserEntitlementsAsync(ulong userId) {
		...
		if (user.ProductAccesses.Count > 0) await UpdateUserFeaturesAsync(user);
	...
	private async Task UpdateUserFeaturesAsync(EliteAccount account) {
		...
		account.UserSettings.Features.HideShopPromotions =
			hasHideShopPromotions && account.UserSettings.Features.HideShopPromotions;
		account.UserSettings.Features.WeightStyleOverride =
			hasWeightStyleOverride && account.UserSettings.Features.WeightStyleOverride;
		account.UserSettings.Features.MoreInfoDefault =
			hasMoreInfoDefault && account.UserSettings.Features.MoreInfoDefault;
		...
		if (account.UserSettings.EmojiUrl is { } emojiUrl) {
			...
			var validEmojiUrl = account.ProductAccesses
				.Any(x =>
					x.IsActive
					&& x.Product.WeightStyles.Exists(s => s.NameStyle?.Emojis.Exists(e => e.Url == emojiUrl) is true)
				);
	...
	private async Task UpdateGuildFeaturesAsync(Guild guild, List<GuildEntitlement> entitlements) {
		...
		if (currentLeaderboards == maxLeaderboards && currentEvents == maxEvents) return;
	...
	public async Task SyncDiscordEntitlementsAsync(ulong entityId, bool isGuild) {
		...
		foreach (var discordEntitlement in discordEntitlements) {
			...
			if (discordEntitlement.Deleted) access.Revoked = discordEntitlement.Deleted;

			if (access.Revoked) order.Status = OrderStatus.Refunded;
		}
		...
		if (account is not null) await UpdateUserFeaturesAsync(account);
	}
...
public class DiscordEntitlement
{
	...
	[JsonPropertyName("sku_id")] public ulong ProductId { get; set; }
