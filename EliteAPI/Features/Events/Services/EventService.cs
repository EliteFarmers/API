...
public class EventService(
	...
	public async Task<ActionResult<Event>> CreateEvent(CreateEventDto eventDto, ulong guildId) {
		...
		if (guild is null) return new BadRequestObjectResult("Guild not found");
		...
		if (endTime is null || startTime is null) return new BadRequestObjectResult("Invalid start or end time");

		if (startTime > endTime) return new BadRequestObjectResult("Start time cannot be after end time");

		if (startTime < DateTimeOffset.UtcNow) return new BadRequestObjectResult("Start time cannot be in the past");
		...
		if (eventDto.MaxTeams < -1) return new BadRequestObjectResult("Invalid max teams");

		if (eventDto.MaxTeamMembers < -1) return new BadRequestObjectResult("Invalid max members");
		...
		if (eliteEvent is null) return new BadRequestObjectResult("Invalid event type");

		if (eliteEvent.Value is null) return eliteEvent;
	...
	public async Task<ActionResult<EventMember>> CreateEventMember(Event eliteEvent,
		CreateEventMemberDto eventMemberDto) {
	...
	public bool CanCreateEvent(Guild guild, [NotNullWhen(false)] out string? reason) {
		...
		if (guild.Features.EventSettings is not null) {
			...
			var startOfMonth = new DateTimeOffset(DateTimeOffset.UtcNow.Year, DateTimeOffset.UtcNow.Month, 1, 0, 0, 0,
				TimeSpan.Zero);
	...
	private async Task<ActionResult<EventMember>> CreateWeightEventMember(Event weightEvent,
		CreateEventMemberDto eventMemberDto) {
	...
	private async Task<ActionResult<EventMember>> CreateMedalsEventMember(Event medalEvent,
		CreateEventMemberDto eventMemberDto) {
	...
	private async Task<ActionResult<EventMember>> CreatePestsEventMember(Event pestEvent,
		CreateEventMemberDto eventMemberDto) {
	...
	private async Task<ActionResult<EventMember>> CreateCollectionEventMember(Event pestEvent,
		CreateEventMemberDto eventMemberDto) {
