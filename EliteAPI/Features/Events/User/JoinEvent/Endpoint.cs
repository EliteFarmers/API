	...
	: Endpoint<JoinEventRequest>
{
	...
	public override async Task HandleAsync(JoinEventRequest request, CancellationToken c) {
		...
		if (userGuild is null) ThrowError("You need to be in the event's Discord server in order to join!");
		...
		if (selectedAccount is null) ThrowError("You need to have a linked Minecraft account in order to join!");

		var member = await context.EventMembers.AsNoTracking()
			.Include(e => e.ProfileMember)
			.ThenInclude(p => p.MinecraftAccount)
			.AsNoTracking()
			.FirstOrDefaultAsync(e => e.EventId == request.EventId && e.ProfileMember.PlayerUuid == selectedAccount.Id,
				c);
		...
		if (updateQuery is null)
			ThrowError(
				"Profile member data not found, please try again later or ensure an existing profile is selected.");
		...
		var profileMember = request.ProfileUuidFormatted.IsNullOrEmpty()
			? await query.FirstOrDefaultAsync(a => a.IsSelected, c)
			: await query.FirstOrDefaultAsync(a => a.ProfileId == request.ProfileUuidFormatted, c);
		...
		if (eventActive != eliteEvent.Active) eliteEvent.Active = eventActive;
