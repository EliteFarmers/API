...
internal sealed class SetEventBannerRequest : DiscordIdRequest {
	...
	[FromForm] public required EditEventBannerDto Data { get; set; }
}
...
internal sealed class SetEventBannerAdminEndpoint(
	...
	public override void Configure() {
		...
		Summary(s => { s.Summary = "Set Custom Event Banner"; });
	}
	...
	public override async Task HandleAsync(SetEventBannerRequest request, CancellationToken c) {
		...
		if (request.Data.Image is null) ThrowError("No image was provided.");

		var newImage = await objectStorageService.UploadImageAsync(
			$"guilds/{request.DiscordIdUlong}/events/{eliteEvent.Id}/{Guid.NewGuid().ToString()}.png",
			request.Data.Image,
			token: c
		);
