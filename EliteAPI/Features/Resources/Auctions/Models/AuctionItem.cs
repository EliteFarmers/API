...
public class AuctionItemVariation
{
	...
	public string ToKey() {
		...
		if (Rarity is not null) list.Add("r:" + Rarity);

		if (Pet is not null) list.Add($"pet:{Pet}");
		...
		if (Extra is not null) list.AddRange(Extra.Select(extra => $"ex:{extra.Key}={extra.Value}"));
		...
		foreach (var part in parts) {
			...
			switch (key) {
				...
				case "pet_group":
					...
					if (groupParts.Length == 2 && int.TryParse(groupParts[0], out var min) &&
					    int.TryParse(groupParts[1], out var max))
						variation.PetLevel ??= new PetLevelGroup
							{ Key = "LVL_" + min + "-" + max, Min = min, Max = max };
					else if (int.TryParse(groupParts[0], out var singleLevel))
						variation.PetLevel ??= new PetLevelGroup
							{ Key = "LVL_" + singleLevel, Min = singleLevel, Max = singleLevel };
					break;
				case "en":
