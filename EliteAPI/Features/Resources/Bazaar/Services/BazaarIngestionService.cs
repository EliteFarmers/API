    ...
    ILogger<BazaarIngestionService> logger) 
{
    ...
    public async Task CalculateAndStoreAveragesAsync(List<string> productIdsToUpdate, DateTimeOffset currentTime, TimeSpan lookbackDuration = default) {
        ...
        foreach (var productId in productIdsToUpdate) {
            ...
            if (existingAveragesDict.TryGetValue(productId, out var existingAverage)) {
                ...
                item ??= new SkyblockItem(itemId: productId);
