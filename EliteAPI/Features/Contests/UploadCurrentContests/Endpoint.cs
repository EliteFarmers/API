...
internal sealed class UploadCurrentContestsEndpoint(
	...
	public override async Task HandleAsync(Dictionary<long, List<string>> request, CancellationToken ct) {
		...
		if (bodyKeys.Count != 124) ThrowError("Invalid number of contests! Expected 124, got " + bodyKeys.Count);
		...
		if (!await db.KeyExistsAsync(lastYearKey)) {
			...
			var contests = await context.JacobContests
				...
				.ToDictionaryAsync(k => k.Timestamp, v => v.Crops, ct);
		...
		try {
			...
			if (lastYearContestsParsed is not null && lastYearContestsParsed.Count > 0) {
				...
				if (allEqual) ThrowError("Contests are the same as last year!");
			}
		}
		catch (Exception e) {
		...
		if (string.IsNullOrEmpty(ipAddress)) ThrowError("Invalid request");
