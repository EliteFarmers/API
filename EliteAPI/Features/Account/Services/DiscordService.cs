	...
	: IDiscordService
{
	...
	public async Task<List<GuildMember>> FetchUserGuilds(ApiUser user) {
		...
		try {
			...
			foreach (var guild in list) {
				...
				if (member is not null) memberList.Add(member);
			}
	...
	private async Task<GuildMember?> UpdateGuildMember(ulong userId, DiscordGuild guild) {
		...
		if (existingGuild is null) {
			existingGuild = await GetGuild(guild.Id, true);

			if (existingGuild is null) return null;
		}
	...
	public async Task<GuildMember?> GetGuildMemberIfAdmin(ClaimsPrincipal user, ulong guildId,
		GuildPermission permission = GuildPermission.Role) {
		...
		if (member is null || !member.HasGuildAdminPermissions(permission)) return null;
	...
	public async Task<GuildMember?> GetGuildMember(ClaimsPrincipal user, ulong guildId) {
		...
		if (member.LastUpdated.OlderThanSeconds(_coolDowns.UserRolesCooldown)) await FetchUserRoles(member);
	...
	public async Task RefreshDiscordGuild(ulong guildId, bool replaceImages = false, int cooldown = -1) {
		await GetGuild(guildId, cooldown == -1, replaceImages, cooldown);
	}
	...
	private async Task<Guild?> UpdateDiscordGuild(FullDiscordGuild? incoming, bool fetchChannels = true,
		...
		if (guild is null) {
			var icon = incoming.Icon is not null
				? await guildImageService.UpdateGuildIconAsync(guildId, incoming.Icon)
				: null;
			...
			if ((replaceImages || guild.Icon?.Hash != incoming.Icon) && incoming.Icon is not null) {
				if (guild.Icon is null)
					guild.Icon =
						await guildImageService.UpdateGuildIconAsync(guildId, incoming.Icon, force: replaceImages);
				else
					await guildImageService.UpdateGuildIconAsync(guildId, incoming.Icon, guild.Icon, replaceImages);
			}

			if (guild.IsPublic && (replaceImages || guild.Banner?.Hash != incoming.Splash) &&
			    incoming.Splash is not null) {
				if (guild.Banner is null)
					guild.Banner =
						await guildImageService.UpdateGuildBannerAsync(guildId, incoming.Splash, force: replaceImages);
				else
					await guildImageService.UpdateGuildBannerAsync(guildId, incoming.Splash, guild.Banner,
						replaceImages);
			}
			...
			if (guild.Features.VerifiedRoleEnabled) guild.Features.VerifiedRole ??= new VerifiedRoleFeature();
		}
	...
	private DiscordUpdateResponse? ProcessResponse(RefreshTokenResponse? tokenResponse) {
		if (tokenResponse?.AccessToken is null || tokenResponse.RefreshToken is null ||
		    tokenResponse.Error is not null) return null;
...
public static class DiscordExtensions
{
	...
	public static bool HasGuildAdminPermissions(this GuildMember member,
		...
		if ((bits & Admin) == Admin) return true;

		// Accept manage guild as admin if permission is set to manager
		if (permission == GuildPermission.Manager) return (bits & ManageGuild) == ManageGuild;

		var adminRole = member.Guild?.AdminRole ?? 0;
		// Accept admin role as admin if permission is set to role
		if (permission == GuildPermission.Role && adminRole != 0 && member.Roles.Contains(adminRole)) return true;
